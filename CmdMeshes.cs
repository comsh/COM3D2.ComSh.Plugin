        ...
        if(name==""){
            ...
            if(ReferenceEquals(tex0.GetType(),typeof(Texture2D))){
                if(cube==1){
                    ...
                    Texture2D tex=new Texture2D(tex0.width,tex0.height,((Texture2D)tex0).format,((Texture2D)tex0).mipmapCount>1);
                    Graphics.CopyTexture(tex0,tex); // 5.6にはTexture.isReadableがないので常にコピーする
                    tex.wrapMode=(wrap==1)?TextureWrapMode.Repeat:TextureWrapMode.Clamp;
