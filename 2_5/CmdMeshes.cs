...
public static class CmdMeshes {
    ...
    public static void PrintShaderProps(ComShInterpreter sh,Material mate,string[] def=null){
        string[] names=mate.GetPropertyNames(MaterialPropertyType.Float);
        for(int i=0; i<names.Length; i++){
            float f=mate.GetFloat(names[i]);
            sh.io.Print(names[i]).Print(':').PrintLn(sh.fmt.FVal(f));
        }
        names=mate.GetPropertyNames(MaterialPropertyType.Int);
        for(int i=0; i<names.Length; i++){
            float f=mate.GetInt(names[i]);
            sh.io.Print(names[i]).Print(':').PrintLn(sh.fmt.FVal(f));
        }
        names=mate.GetPropertyNames(MaterialPropertyType.Vector);
        for(int i=0; i<names.Length; i++){
            if( names[i].EndsWith("_HDR",StringComparison.Ordinal)
             || names[i].EndsWith("_ST",StringComparison.Ordinal)
             || names[i].EndsWith("_TexelSize",StringComparison.Ordinal)) continue;
            var v=mate.GetVector(names[i]);
            sh.io.Print(names[i]).Print(':').PrintLn(sh.fmt.RGBA(v));
        }
    ...
    private static int MeshParamTexture(ComShInterpreter sh,SingleMesh sm,string val){
        ...
        if(val==null){
            ...
            var names=mate.GetTexturePropertyNames();
            for(int i=0; i<names.Length; i++) PrintTextureInfo(sh,mate,names[i]);
            return 0;
        }
        if(val=="") return 0;
