...
public static class CmdLights {
    ...
    private static int CmdLight(ComShInterpreter sh,List<string> args){
        ...
        if(args.Count==1){
            ...
            if(sh.lightRef.Length>0){     // スタジオモード分参照
                ...
                if(pftr!=null) for(int i=0; i<pftr.childCount; i++){
                    ...
                    if(!lt.enabled) type="disabled"; else type=lt.type==LightType.Point?"point":"spot";
                    lset.Add(tr.name);
        ...
        if(args[1]=="add"){
            ...
            string[] pa=ParseUtil.NormalizeParams(args,new string[]{null,"","ffffff","1","1","0,1,0","0,-1,0","30"},2);
            ...
            if((pftr=LightUtil.GetLightObjectTr(sh,true))==null) return sh.io.Error("ライト作成に失敗しました"); // 想定外
            float[] pos,col;
            ...
            if(pa[0]=="point"){
                ...
                LightUtil.lightDic.Add(pa[1],go.transform);
            }else return sh.io.Error("種別にはpointかspotを指定してください");
            return 0;
        }
    ...
    public static int CmdLightSub(ComShInterpreter sh,string id,List<string> args,int prmstart){
        ...
        Transform pftr,tr;
        if((pftr=LightUtil.GetLightObjectTr(sh,true))==null) return sh.io.Error("ライトが存在しません");
...
public static class LightUtil{
    ...
    public static Transform GetLightObjectTr(ComShInterpreter sh,bool create=false){
        if(sh.lightBase==string.Empty){
            GameObject bg=GameMain.Instance.BgMgr.BgObject;
            if(bg!=null) return bg.transform;
            return null;
        }else return UTIL.GetObjRoot(sh.lightBase,create);
    }
