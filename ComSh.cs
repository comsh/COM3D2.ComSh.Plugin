        ...
        private void TerminalEvent() {
            ...
            if(e.type==EventType.KeyDown){
                ...
                if(e.type==EventType.MouseDown){
                    ...
                    if(!logTe.hasSelection) CmdFocus(); else{ logTe.Copy(); LogFocus(); }
		...
		public void Draw() {
            if(PanelStyleCache.updDate>styleDate){
                ...
                float width=ComShProperties.fontSize*10,
                    w=ws.title.CalcSize(new GUIContent(title)).x+ws.cbSize.x*2;
                if(w>width) width=w;
                for(int i=0; i<items.Length; i++){
                    w=bstyle.CalcSize(new GUIContent(items[i][0])).x;
                    if(w>width) width=w;
                }
